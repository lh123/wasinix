...
#
# The fork's vendoring auto-tracks the src lockfiles (no FOD hash), so the only
# pin that can drift is the stage0 bootstrap compiler, and its version/url/hash
# are dictated by the fork's own src/stage0 at the new tag. That is package
# knowledge, not driver knowledge, so it lives next to the pin it edits.
#
...
from updater_lib import REPO, prefetch_url, raw_file, run_nix_update  # noqa: E402
